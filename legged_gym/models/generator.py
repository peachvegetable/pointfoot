...
class TransformerGenerator(nn.Module):
    def __init__(self, noise_dim, hidden_dim, output_range, num_layers=2, num_heads=4):
        super(TransformerGenerator, self).__init__()
        self.embedding = nn.Linear(noise_dim, hidden_dim)
    ...
    def forward(self, x):
        ...
        x = (x + 1) / 2  # Scaling to [0, 1]
        x = x * (self.output_range[1] - self.output_range[0]) + self.output_range[0]  # Scaling to the desired range
        return x
